...
export function parseTableEditTag(piece, mesIndex = -1, ignoreCheck = false) {
  ...
  tableEditActions.forEach(
    (action, index) => (tableEditActions[index].action = classifyParams(formatParams(action.param))),
  );
  console.log('解析到的表格编辑指令', tableEditActions);

  // 获取上一个表格数据
  const { piece: prePiece } =
    mesIndex === -1 ? BASE.getLastSheetsPiece(1) : BASE.getLastSheetsPiece(mesIndex - 1, 1000, false);
...
async function onChatCompletionPromptReady(eventData) {
  try {
    ...
    if (USER.tableBaseSetting.deep === 0) eventData.chat.push({ role: getMesRole(), content: promptContent });
    else eventData.chat.splice(-USER.tableBaseSetting.deep, 0, { role: getMesRole(), content: promptContent });
...
function getMacroTablePrompt() {
  try {
    ...
    const lastSheetsPiece = BASE.getReferencePiece();
    if (!lastSheetsPiece) return '';
    const promptContent = getTablePromptByPiece(lastSheetsPiece, ['title', 'node', 'headers', 'rows']);
    return replaceUserTag(promptContent);
  } catch (error) {
    EDITOR.error(`记忆插件：宏提示词注入失败\n原因：`, error.message, error);
    return '';
